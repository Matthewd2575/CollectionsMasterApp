...
namespace CollectionsMasterConsoleUI
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"{numbers[numbers.Length - 1]}"); // ($"The last number is {numbers.Length - 1}");
            ...
            var sorted = numbers.OrderBy(x => x); // Array.Sort(numbers); NumberPrinter(numbers);
            ...
            Console.WriteLine($"Capacity{numList.Count}"); //"$"Capacity: {numList.Capacity}"
        ...
        {
            for(int i = 0; i < numbers.Length; i++) // foreach (var i in numbers)
            {
                if(numbers[i] % 3 == 0)           // if ( i % 3==0)
                {                                 //{number[i] = 0 }
                    numbers[i] = 0;               //
                }
            }
